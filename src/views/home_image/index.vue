  ...
  <div class="app-container">
    ...
    <el-form :inline="true" :model="searchDept" class="demo-form-inline">
      <el-form-item label="名称">
        <el-input v-model="searchDept.name" placeholder="请输入图片名称"></el-input>
      </el-form-item>
      <el-form-item>
    ...
    <el-row>
      ...
      <el-button type="primary" size="medium" @click="openDialog">+ 图片插入</el-button>
    </el-row>

    <!--添加数据对话框表单-->
    <el-dialog ref="form" title="图片插入" :visible.sync="dialogFormVisible" width="50%">
      <el-form ref="form" :model="home" label-width="80px" size="mini">
        <el-form-item label="名称">
        ...
        <el-form-item label="图片">
          <el-upload class="avatar-uploader" action="/api/upload/lunbotu" :headers="token" name="image"
            ...
            <img v-if="home.image" :src="home.image" class="avatar" />
       ...
       <!--  <el-form-item>
          <div>
            <el-tiptap id="test" ref="editor" v-model="news.content" :extensions="extensions" />
          </div>
        </el-form-item> -->
        <el-form-item>
...
<script>
import { findAll, page, add, update, deleteById, selectById } from "@/api/home_image.js"
...
created() {
      ...
      extensions: [
        ...
        new Image({
          ...
          uploadRequest(file) {
            ...
            // 发送图片上传请求到内容接口
            return fetch('api/upload/lunbotu', {
              method: 'POST',
              body: formData,
              headers: {
                token:getToken()
  ...
  methods: {
    ...
    add() {
      let method;// 添加
      if (this.home.id) {
        method = update(this.home); // 修改
      } else {
        method = add(this.home); //添加 
      }
      method.then((result) => {
