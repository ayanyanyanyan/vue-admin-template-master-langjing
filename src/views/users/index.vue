  ...
  <div class="app-container">
    ...
    <el-form :inline="true" :model="searchEmp" class="demo-form-inline">
      ...
      <el-form-item label="创建时间">
        <el-date-picker v-model="entrydate" clearable value-format="yyyy-MM-dd" type="daterange" placeholder="选择日期"
    ...
    <el-row>
      ...
      <el-button type="primary" size="medium" @click="dialogVisible = true; users = { image: '' };">+ 新增用户</el-button>
    </el-row>
    ...
    <el-dialog ref="form" title="编辑用户" :visible.sync="dialogVisible" width="30%">
      <el-form ref="form" :model="users" label-width="80px" size="mini">
        ...
        <el-form-item label="头像">
          <el-upload class="avatar-uploader" action="/api/upload/user" :headers="token" name="image"
    ...
    <el-dialog ref="form" title="编辑用户" :visible.sync="dialogVisible1" width="30%">
      <el-form ref="form" :model="users" label-width="80px" size="mini">
        <el-form-item label="姓名">
          <el-input v-model="users.name"></el-input>
        </el-form-item>
    ...
    <template>
      <el-table :data="tableData" style="width: 100%" border @selection-change="handleSelectionChange">
        ...
        <el-table-column align="center" label="最后操作日期">
          <template slot-scope="scope">
...
//         const rawData = res.data.data.rows; // 原始数据数组
//         const selectedTimeRangeData = []; // 存储符合时间段条件的数据的数组

//         for (const item of rawData) {
//           const createTime = new Date(item.createTime); // 将 createTime 字符串转换为日期对象
//           console.log(item.createTime);
          
//           // console.log(item.this.endTime);
//           if (
//             (!this.beginTime || createTime >= this.beginTime) && // 开始时间条件
//             (!this.endTime || createTime <= this.endTime) // 结束时间条件
//           ) {
//             selectedTimeRangeData.push(item); // 将符合条件的数据添加到 selectedTimeRangeData 数组中
//           }
//         }

//         console.log(selectedTimeRangeData); // 输出符合时间段条件的数据
//         this.totalCount = selectedTimeRangeData.length; // 更新总记录数
//         this.tableData = selectedTimeRangeData; // 更新表格数据
//         console.log(this.tableData);
//         console.log(this.value1[0]);
//         console.log(this.value1[1]);
// if(item.createTime<this.value1[1])
// {
//   console.log("111111");
// }
      });
