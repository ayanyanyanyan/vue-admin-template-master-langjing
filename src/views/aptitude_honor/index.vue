  ...
  <div class="app-container">
    ...
    <el-form :inline="true" :model="searchDept" class="demo-form-inline">
      <el-form-item label="标题">
        <el-input v-model="searchDept.name" placeholder="请输入"></el-input>
      </el-form-item>
      <el-form-item>
    ...
    <el-row>
      ...
      <el-button type="primary" size="medium" @click="openDialog">+ 资质更新</el-button>
    </el-row>

    <!--添加数据对话框表单-->
    <el-dialog ref="form" title="资质" :visible.sync="dialogFormVisible" width="50%">
      <el-form ref="form" :model="honor" label-width="80px" size="mini">
        <el-form-item label="名称">
          <el-input v-model="honor.name"></el-input>
        </el-form-item>
        <el-form-item label="图片">
          <el-upload class="avatar-uploader" action="/api/upload/honor" :headers="token" name="image"
            :show-file-list="false" :on-success="handleAvatarSuccess" :before-upload="beforeAvatarUpload">
            <img v-if="honor.image" :src="honor.image" class="avatar" />
        ...
        <el-form-item>
          <div>
            <el-tiptap id="test" ref="editor" v-model="honor.content" :extensions="extensions" />
          </div>
        </el-form-item>
        <el-form-item>
...
<script>
import { findAll, page, add, update, deleteById, selectById } from "@/api/aptitude_honor.js"
...
created() {
  ...
  methods: {
    ...
    add() {
      let method;// 添加
      if (this.honor.id) {
        method = update(this.honor); // 修改
      } else {
        method = add(this.honor); //添加 
      }
      method.then((result) => {
