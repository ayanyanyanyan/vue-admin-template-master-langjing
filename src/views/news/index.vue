  ...
  <div class="app-container">
    ...
    <el-form :inline="true" :model="searchDept" class="demo-form-inline">
      <el-form-item label="标题">
        <el-input v-model="searchDept.name" placeholder="请输入新闻标题"></el-input>
      </el-form-item>
      <el-form-item>
    ...
    <el-row>
      ...
      <el-button type="primary" size="medium" @click="openDialog">+ 新闻发布</el-button>
    </el-row>

    <!--添加数据对话框表单-->
    <el-dialog ref="form" title="新闻发布" :visible.sync="dialogFormVisible" width="50%">
      <el-form ref="form" :model="news" label-width="80px" size="mini">
...
<script>
import { findAll, page, add, update, deleteById, selectById } from "@/api/news.js"
...
created() {
  ...
  methods: {
    ...
    add() {
      let method;// 添加
      if (this.news.id) {
        method = update(this.news); // 修改
      } else {
        method = add(this.news); //添加 
      }
      method.then((result) => {
